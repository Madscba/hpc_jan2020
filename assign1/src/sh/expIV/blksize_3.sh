...
cd "$RPATH/$SRCPATH"
mdim="413"
BLOCKSIZE=$(seq 63 10 143)
/bin/rm -f "$RPATH/$DPATH/blk3_1$LOGEXT"
for bs in $BLOCKSIZE
do
	...
	./$EXECUTABLE blk $mdim $mdim $mdim $bs | grep -v CPU >> $RPATH/$DPATH/blk3_1$LOGEXT
done
mdim="146"
BLOCKSIZE=$(seq 63 10 143)
/bin/rm -f "$RPATH/$DPATH/blk3_2$LOGEXT"
for bs in $BLOCKSIZE
do
	echo ./$EXECUTABLE blk $mdim $bs
		#collect -o $EXPOUT $HWCOUNT ./$EXECUTABLE $perm $mdim $mdim $mdim | grep -v CPU >> $RPATH/$DPATH/ratio_1_$perm.$LOGEXT
	./$EXECUTABLE blk $(($mdim*4)) $(($mdim*4)) $mdim $bs | grep -v CPU >> $RPATH/$DPATH/blk3_2$LOGEXT
done

/bin/rm -f "$RPATH/$DPATH/blk3_3$LOGEXT"
mdim="238"
BLOCKSIZE=$(seq 63 10 143)
for bs in $BLOCKSIZE
do
	echo ./$EXECUTABLE blk $mdim $bs
		#collect -o $EXPOUT $HWCOUNT ./$EXECUTABLE $perm $mdim $mdim $mdim | grep -v CPU >> $RPATH/$DPATH/ratio_1_$perm.$LOGEXT
	./$EXECUTABLE blk $mdim $mdim $(($mdim*4)) $bs | grep -v CPU >> $RPATH/$DPATH/blk3_3$LOGEXT
done
