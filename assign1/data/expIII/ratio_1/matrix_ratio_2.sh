...
do
	...
	do
		echo ./matmult_c.${CC} $perm $mdim
    		collect -o $EXPOUT$perm.er $HWCOUNT ./$EXECUTABLE $perm $(($mdim*4)) $(($mdim*4)) $mdim | grep -v CPU >> $RPATH/$DPATH/ratio_2_$perm.$LOGEXT
		#./$EXECUTABLE $perm $(($mdim*4)) $(($mdim*4)) $mdim | grep -v CPU >> $RPATH/$DPATH/ratio_2_$perm.$LOGEXT
	done
done
