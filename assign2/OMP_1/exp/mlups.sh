...
do
	...
	echo $EXECUTABLE__J  $NDIM 1000 0.0 $START_T 0 1
	$EXECUTABLE_J $NDIM 1000 0.0 $START_T 0 1  | grep -v CPU >> ./perf_j$LOGEXT

	
	echo $EXECUTABLE__GS  $NDIM 1000 0.0 $START_T 0 1
	$EXECUTABLE_GS $NDIM 1000 0.0 $START_T 0 1  | grep -v CPU >> ./perf_gs$LOGEXT
done
JID=${LSB_JOBID}
